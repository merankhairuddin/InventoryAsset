...
# Webhook URL for the channel where inventory JSON will be sent
# (Create this in your Discord channel's Integrations → Webhooks)
...
# Optional: override webhook username/icon in messages
$DiscordUsername   = "Asset Inventory Bot"
$DiscordAvatarUrl  = ""  # or a public image URL if you want an avatar
# ============================================================
...
# -------------------------------------------------------------------
# Helper: interactive override + validation + technician (Win7 safe)
# -------------------------------------------------------------------
function Ask-Override {
    ...
    # Windows 7 friendly: no [string]::IsNullOrWhiteSpace()
...
function Ask-DateOptional {
    ...
    while ($true) {
        ...
        # Treat null/empty/whitespace as "no change"
...
function Get-Technician {
    ...
    # Windows 7 friendly: simple null/empty check
    ...
    try {
        if (Get-Command Get-NetAdapter -ErrorAction SilentlyContinue) {
            ...
            foreach ($a in $adapters) {
                $ipList = @()
                if ($a.IPAddress) {
                    $ipList = $a.IPAddress | Where-Object { $_ -notlike "169.254.*" -and $_ -like "*.*" }
                }
                ...
                if ($a.MACAddress) {
                    $info.mac_addresses += $a.MACAddress
                }
            }
        }
...
# -------------------------------------------------------------------
# Discord upload helper (HttpClient, Win7+ with .NET 4.5+)
# -------------------------------------------------------------------
